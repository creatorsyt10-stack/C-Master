...
async function runCode() {
  ...
  debugPane.textContent = `Language: ${language.name}\nFile: ${language.file}\nRunner: ${language.local ? "Browser local" : "Piston remote API"}\nStarted: ${new Date().toLocaleTimeString()}`;
  ...
  try {
    ...
    if (language.id === "javascript") {
      ...
      result = await runWithPiston(language, code, stdin);
    }
...
async function runWithPiston(language, code, stdin) {
  try {
    ...
    if (!response.ok) {
      return await runLocalFallback(language, code, stdin, `Remote compiler returned HTTP ${response.status}, so CMaster used the browser fallback runner.`);
    }
    const data = await response.json();
    const compile = data.compile || {};
    const run = data.run || {};
    return {
      output: [compile.stdout, run.stdout].filter(Boolean).join("\n"),
      error: [compile.stderr, run.stderr, run.signal ? `Signal: ${run.signal}` : ""].filter(Boolean).join("\n")
    };
  } catch (error) {
    return await runLocalFallback(language, code, stdin, `Remote compiler is unreachable, so CMaster used the browser fallback runner. Detail: ${error.message}`);
  }
}
